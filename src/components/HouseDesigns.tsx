  ...
  return (
    <section id="designs" className="min-h-[60vh] md:h-screen md:min-h-screen w-full bg-gray-900 flex flex-col items-center justify-center relative overflow-hidden py-8 sm:py-10 md:py-4 lg:py-8 xl:py-12 md:border-0 border-0">
      <div className="w-full max-w-[1920px] mx-auto px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 2xl:px-12 flex flex-col items-center justify-center flex-1 overflow-y-auto">
        ...
        <div className="text-primary text-[8px] sm:text-[9px] md:text-[10px] lg:text-xs xl:text-sm 2xl:text-base uppercase tracking-wider mb-1 sm:mb-1.5 md:mb-2 lg:mb-3 xl:mb-4 font-body font-thin text-center w-full px-1 sm:px-0">
          {houseData.slice(0, 4).length} DAY HOUSES IN STOCK
        </div>
        
        {/* Grid layout - fully responsive */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-2 md:gap-2.5 lg:gap-3 xl:gap-4 2xl:gap-6 mb-1.5 sm:mb-2 md:mb-2.5 lg:mb-3 xl:mb-4 w-full">
          {houseData.slice(0, 4).map((house, idx) => {
            return (
            <Link 
              key={idx} 
              to={`/house/${house.slug}`}
              className="bg-gray-800 rounded-lg sm:rounded-xl shadow-lg sm:shadow-xl overflow-hidden flex flex-col md:hover:scale-[1.02] md:hover:scale-105 md:hover:shadow-2xl transition-all duration-300 h-full cursor-pointer"
            >
              ...
              <div className="p-1.5 sm:p-2 md:p-2.5 lg:p-3 xl:p-4 2xl:p-5 flex-1 flex flex-col justify-between">
                <div>
                  <div className="text-[10px] sm:text-xs md:text-sm lg:text-base xl:text-lg 2xl:text-xl text-primary font-thin mb-0.5 sm:mb-0.5 md:mb-1 font-heading uppercase">{house.name}</div>
                  <div className="text-white text-[9px] sm:text-[10px] md:text-xs lg:text-sm xl:text-base mb-0.5 sm:mb-1 md:mb-1.5 font-body font-thin">{house.squareFeet} ft² • {house.type}</div>
                </div>
